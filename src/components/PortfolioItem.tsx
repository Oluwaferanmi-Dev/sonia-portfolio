...
const PortfolioItem: React.FC<{ item: LocalPortfolioItem }> = ({ item }) => {
    ...
    <div className="group overflow-hidden rounded-2xl shadow-md hover:shadow-lg transition-all duration-300">
      <div className="relative overflow-hidden h-64">
        {primaryMedia.type === "image" ? (
          <img
            src={primaryMedia.url || "/placeholder.svg"}
            alt={item.title}
            className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
          />
        ) : (
          <video src={primaryMedia.url} controls className="w-full h-full object-cover" />
        )}
